...
import finalproject.TreeSet;// to be replaced by your own implementation of TreeSet
...
class TestToArray {
	...
	void toArray_str_set() {
		Object[] testArr = ss1to20.toArray();
		for(Object obj:testArr) {
			if (!ss1to20.contains(obj)) {
				fail("test toArray() on set of String objects");		
			}
		}
		if (testArr.length!=ss1to20.size()) {
			fail("test toArray() on set of String objects");
		}
	}
	...
	@Test
	void toArray_pt_set() {
		Object[] testArr = ps1to20.toArray();
		for(Object obj:testArr) {
			if (!ps1to20.contains(obj)) {
				fail("test toArray() on set of custom class objects");		
			}
		}
		if (testArr.length!=ps1to20.size()) {
			fail("test toArray() on set of custom class objects");
		}
	}
